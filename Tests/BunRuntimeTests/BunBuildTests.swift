...
struct BunBuildTests {
    ...
    @Test func bundleLoads() async throws {
        ...
        _ = try await TestProcessSupport.withLoadedProcess(BunProcess(bundle: url)) { _ in
            true
        }
    }
    ...
    @Test func pathOperations() async throws {
        let url = try bundleURL()
        let result = try await TestProcessSupport.withLoadedProcess(BunProcess(bundle: url)) { process in
            try await process.evaluate(js: "JSON.stringify(__testResults.path)")
        }
    ...
    @Test func cryptoUUID() async throws {
        let url = try bundleURL()
        let result = try await TestProcessSupport.withLoadedProcess(BunProcess(bundle: url)) { process in
            try await process.evaluate(js: "__testResults.crypto.uuidLength")
        }
    ...
    @Test func bunGlobalAvailable() async throws {
        let url = try bundleURL()
        let result = try await TestProcessSupport.withLoadedProcess(BunProcess(bundle: url)) { process in
            try await process.evaluate(js: "__testResults.bun.version")
        }
    ...
    @Test func allChecksPass() async throws {
        let url = try bundleURL()
        let result = try await TestProcessSupport.withLoadedProcess(BunProcess(bundle: url)) { process in
            try await process.evaluate(js: "__testResults.ok")
        }
