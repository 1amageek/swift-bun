...
function parse(E2, g = "@") {
  ...
  for (; C.ri(); ) {
    ...
    let K2;
    C.ip() && (K2 = k(E2.slice(-1 === I2 ? A2 - 1 : A2, -1 === I2 ? Q2 + 1 : Q2)));
    const N = [];
    for (C.rsa(); C.ra(); ) {
      const A3 = C.aks(), Q3 = C.ake(), B3 = C.avs(), g3 = C.ave();
      N.push([J(E2.slice(A3, Q3)), J(E2.slice(B3, g3))]);
    }
    w.push({ n: K2, t: B2, s: A2, e: Q2, ss: o2, se: D2, d: I2, a: g2, at: N.length > 0 ? N : null });
  }
  for (; C.re(); ) {
    const A2 = C.es(), Q2 = C.ee(), B2 = C.els(), g2 = C.ele(), I2 = J(E2.slice(A2, Q2)), o2 = B2 < 0 ? void 0 : J(E2.slice(B2, g2));
...
// index.js
var esm = (init_lexer(), __toCommonJS(lexer_exports));
esm.initSync();
globalThis.__transformESM = function(source, bundleURL) {
  const parse2 = esm.parse;
  {
    const [imports, exports2] = parse2(source);
    const ops = [];
      ...
      if (imp.d === -2) {
        ...
        if (fullStmt.trimStart().startsWith("export")) {
          if (fullStmt.includes("*")) {
            let replacement2 = 'require("' + moduleName + '")';
            if (fullStmt.trimEnd().endsWith(";")) replacement2 += ";";
            ops.push({ start: imp.ss, end: imp.se, replacement: replacement2 });
          } else {
            const braceOpen = fullStmt.indexOf("{");
            const braceClose = fullStmt.lastIndexOf("}");
            if (braceOpen >= 0 && braceClose > braceOpen) {
              const specs = fullStmt.slice(braceOpen + 1, braceClose).replace(/\s+as\s+/g, ":");
              let replacement2 = "var{" + specs + '}=require("' + moduleName + '")';
              if (fullStmt.trimEnd().endsWith(";")) replacement2 += ";";
              ops.push({ start: imp.ss, end: imp.se, replacement: replacement2 });
            }
          }
        ...
        const importClause = fromMatch ? fullStmt.slice(6, fullStmt.indexOf(fromMatch[0])).trim() : "";
        let replacement;
        if (!importClause) {
          replacement = 'require("' + moduleName + '")';
        } else if (importClause.startsWith("{")) {
          const specs = importClause.slice(1, importClause.lastIndexOf("}")).replace(/\s+as\s+/g, ":");
          ...
          if (braceOpen >= 0 && braceClose > braceOpen) {
            const specs = rest.slice(braceOpen + 1, braceClose).replace(/\s+as\s+/g, ":");
            replacement = "var " + defaultName + '=require("' + moduleName + '"),{' + specs + "}=" + defaultName;
          } else {
            replacement = "var " + defaultName + '=require("' + moduleName + '")';
          }
        } else {
          replacement = "var " + importClause + '=require("' + moduleName + '")';
        }
        if (fullStmt.trimEnd().endsWith(";")) replacement += ";";
        ops.push({ start: imp.ss, end: imp.se, replacement });
      } else if (imp.d >= 0 && imp.a >= 0) {
        const callStart = imp.d;
        ...
        if (source[commaPos] === ",") {
          ...
          while (firstArgEnd > callStart + 1 && /\s/.test(source[firstArgEnd - 1]))
            firstArgEnd--;
